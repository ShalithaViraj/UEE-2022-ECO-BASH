...
class ViewMoreCon extends StatefulWidget {
  const ViewMoreCon({Key? key, required this.ConKey})
      : super(key: key);

  final String ConKey;

  @override
  State<ViewMoreCon> createState() => _ViewMoreConState();
}

class _ViewMoreConState extends State<ViewMoreCon> {
  ...
  void getorganicData() async {
    DataSnapshot snapshot = await dbRef.child(widget.ConKey).get();

    Map organic = snapshot.value as Map;

    organicidController.text = organic['soilconid'];
    organicnameController.text = organic['soilconname'];
    descriptionController.text = organic['soilcondesd'];
    oragnicImageController.text = organic['soilconimage'];
  }
