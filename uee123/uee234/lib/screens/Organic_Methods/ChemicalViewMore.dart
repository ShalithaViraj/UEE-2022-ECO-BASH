...
class ViewMoreChemical extends StatefulWidget {
  const ViewMoreChemical({Key? key, required this.ChemicalsKey})
      : super(key: key);

  final String ChemicalsKey;

  @override
  State<ViewMoreChemical> createState() => _ViewMoreChemicalState();
}

class _ViewMoreChemicalState extends State<ViewMoreChemical> {
  ...
  void getorganicData() async {
    DataSnapshot snapshot = await dbRef.child(widget.ChemicalsKey).get();

    Map organic = snapshot.value as Map;

    organicidController.text = organic['chemid'];
    organicnameController.text = organic['chemName'];
    descriptionController.text = organic['chemdesd'];
    oragnicImageController.text = organic['chemimage'];
  }
